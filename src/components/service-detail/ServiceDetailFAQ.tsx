  ...
  return (
    <section className="section-pad bg-[#F5F6F8]">
      <div className="container mx-auto gutter-x">
        ...
        <div className="max-w-3xl mx-auto bg-white rounded-xl p-6 sm:p-8 border border-[#00153F]/10 shadow-crisp">
          <Accordion type="single" collapsible className="w-full space-y-4">
            {service.faqs.map((faq, idx) => (
              <AccordionItem
                key={idx}
                value={`item-${idx}`}
                className="border border-[#00153F]/10 rounded-lg px-5 data-[state=open]:border-[#C99A55]/50 data-[state=open]:bg-[#F5EDE0]/20 transition-colors"
              >
                <AccordionTrigger className="text-left font-heading font-bold text-base sm:text-lg text-[#00153F] hover:no-underline hover:text-[#C99A55] py-4">
                  {faq.question}
                </AccordionTrigger>
                <AccordionContent className="text-sm sm:text-base text-muted-foreground leading-relaxed pt-2 pb-5 border-t border-[#00153F]/5">
                  {faq.answer}
                </AccordionContent>
              </AccordionItem>
            ))}
