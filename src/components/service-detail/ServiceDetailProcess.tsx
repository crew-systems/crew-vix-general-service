      ...
      <div className="container mx-auto gutter-x">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {service.process.map((step, idx) => (
            <div
              key={idx}
              className="bg-white rounded-xl p-6 border border-[#00153F]/10 shadow-crisp relative flex flex-col group hover:-translate-y-1 transition-transform duration-300"
            >
              <div className="flex items-center justify-between mb-4">
                <span className="text-3xl font-heading font-extrabold text-[#C99A55]/60 group-hover:text-[#C99A55] transition-colors">
                  {step.step}
                </span>
                <div className="w-8 h-8 rounded-full bg-[#00153F] text-[#EDE4D6] flex items-center justify-center text-xs font-bold">
                  {idx + 1}
                </div>
              </div>
